...
func (e *EventEmitter) On(event string, listener func(*Data)) {
	e.listeners[event] = append(e.listeners[event], listener)
}

func (e *EventEmitter) Emit(event string, data *Data) {
...
func(r *_Room) Connect(){
  ...
  r.Mgr.Event.Emit("Connect", &Data{ Room: r  })
  ...
  r.Auth()
	...
	for {
		...
		if err != nil {
      r.Mgr.Event.Emit("Disconnect", &Data{ Room: r, ErrorMessage: err })
			return
		}
...
func (r *_Room) _rcmd_b(args []string){
  ...
  r.Mgr.Event.Emit("Message", &Data{ User: user, Room: r, Message: msg})
}
